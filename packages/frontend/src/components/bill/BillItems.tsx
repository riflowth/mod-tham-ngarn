  ...
  const rowElements = rows.map((row, i) => {
    return (
      <TableRow key={row.billId}>
        ...
        <TableCell key={i} style={{ width: 160 }}>{row.billId}</TableCell>
        <TableCell key={i} style={{ width: 160 }}>{row.storeName}</TableCell>
        <TableCell key={i} style={{ width: 160 }}>{row.orderBy}</TableCell>
